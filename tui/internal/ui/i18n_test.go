...
func TestLocalizerLoadsEnglishCatalog(t *testing.T) {
	...
	want := "message not sent - the agent is still starting; press Enter to retry"
	if got != want {
...
func TestLocalizerFallsBackToEnglishForUnknownLocale(t *testing.T) {
	...
	want := "message not sent - no agent is configured; configure or start an agent first"
	if got != want {
...
func TestLocalizerLoadsSpanishCatalog(t *testing.T) {
	...
	got := l.t(msgPostFailureAgentNotConfigured, nil)
	if !strings.Contains(got, "no hay ningún agente configurado") {
