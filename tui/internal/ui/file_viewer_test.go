...
func TestFileViewerImageDetailRendersWithChafaWhenAvailable(t *testing.T) {
	...
	modes := a.localFileDetailModes(fileTreeEntry{Path: "plot.png", Size: 6}, path)
...
func TestFileViewerEnterOnRightSidebarImageOpensRenderedPreview(t *testing.T) {
	...
	if !a.detailViewOpen || a.detailView == nil || a.detailView.fileMode != "image" {
		t.Fatalf("enter should open image detail, open=%v detail=%+v", a.detailViewOpen, a.detailView)
	}
	if !strings.Contains(a.detailView.fullText, "IMAGE PREVIEW FROM ENTER") {
...
func TestFileViewerEnterOnRightSidebarLargeCSVOpensTablePreview(t *testing.T) {
	...
	a := NewWithTheme("http://unused", ThemeForMode(ModeDark))
	a.SetFileViewerRoot(root)
	a.fileTreeEntries = []fileTreeEntry{{Path: "large.csv", Name: "large.csv", Size: maxLocalFilePreviewBytes + 1}}
	a.focus = FocusRightSidebar
	a.sidebarHitFocus = FocusRightSidebar
	a.sidebarSectionFocus = sidebarSectionFiles
	a.sidebarSectionCursor = false
	...
	if !a.detailViewOpen || a.detailView == nil || a.detailView.fileMode != "table" {
		t.Fatalf("enter should open csv table detail, open=%v detail=%+v", a.detailViewOpen, a.detailView)
	}
	if !strings.Contains(a.detailView.fullText, "MTA1") || !strings.Contains(a.detailView.fullText, "2 data rows total") {
		t.Fatalf("large csv detail missing table preview:\n%s", a.detailView.fullText)
	}
	if strings.Contains(a.detailView.fullText, "inline preview limit") {
...
func TestFileViewerPDFDetailUsesInstalledTextRenderer(t *testing.T) {
	...
	modes := a.localFileDetailModes(fileTreeEntry{Path: "report.pdf", Size: 9}, path)
...
func TestFileViewerScientificBinaryDetailUsesAvailableRenderer(t *testing.T) {
	...
	h5Modes := a.localFileDetailModes(fileTreeEntry{Path: "sample.h5", Size: 3}, h5Path)
	...
	parquetModes := a.localFileDetailModes(fileTreeEntry{Path: "sample.parquet", Size: 3}, parquetPath)
...
func TestFileViewerRendererFallbackDoesNotAdvertiseFutureWork(t *testing.T) {
	...
	modes := a.localFileDetailModes(fileTreeEntry{Path: "report.html", Size: 15}, path)
