...
func (a *App) fullConversationTextCached() (string, bool) {
	if a == nil {
		return fullConversationText(nil)
	}
	key := fullConversationCopyCacheKey(a.messages)
	if a.fullConversationCopyCache.valid && key == a.fullConversationCopyCache.key {
		return a.fullConversationCopyCache.text, a.fullConversationCopyCache.ok
	}
	text, ok := fullConversationText(a.messages)
	a.fullConversationCopyCache = fullConversationCopyCache{
		key:   key,
		valid: true,
		text:  text,
		ok:    ok,
