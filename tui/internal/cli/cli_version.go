...
var (
	...
	// `go build`. The version.BuildRevision/BuildTime/BuildDirty -ldflags vars
	// still drive the detailed `revision:` line.
...
func readVCSInfo() (rev, when string, dirty bool) {
	if strings.TrimSpace(version.BuildRevision) != "" {
		rev = strings.TrimSpace(version.BuildRevision)
		if len(rev) > 12 {
			rev = rev[:12]
		}
		return rev, strings.TrimSpace(version.BuildTime), strings.EqualFold(strings.TrimSpace(version.BuildDirty), "true")
	}
