...
func runRewind(args []string) int {
	var includeTarget *bool
	cc, rest, code := newCmdCtx("rewind", args, withFlags(func(fs *flag.FlagSet) {
...
func runUndo(args []string) int {
	var count *int
	cc, rest, code := newCmdCtx("undo", args, withFlags(func(fs *flag.FlagSet) {
