...
// defaultBackend/defaultTheme are the built-in defaults for CLI subcommands.
// The single source of truth lives in internal/config (shared with the
// interactive TUI entry point); these are package-local aliases so the ~40
// subcommands can keep referencing the short names without each importing
// config.
const (
