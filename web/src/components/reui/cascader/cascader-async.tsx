...
) => CascaderNode<T>[] | CascaderLoadResult<T> | Promise<CascaderNode<T>[] | CascaderLoadResult<T>>;
...
export function useCascaderLoader<T = unknown>({
  ...
  const [search, setSearch] = React.useState<CascaderSearchSlice<T> | null>(null);
  ...
  const runLoad = React.useCallback(
    (key: string, reason: CascaderLoadReason, cursor: string | undefined, append: boolean) => {
      ...
      const settle = () => {
        if (inflight.current.get(key) === signature) inflight.current.delete(key);
      ...
      Promise.resolve()
        ...
        .then((result) => {
          ...
          const nextCursor = Array.isArray(result) ? undefined : result.nextCursor;
          const hasMore = Array.isArray(result) ? false : (result.hasMore ?? nextCursor != null);

          if (!append) moreLatch.current.delete(key);
          remember(items);
          setStore((prev) => withPage(prev, key, items, { append, hasMore, cursor: nextCursor }));
        })
        .catch((error: unknown) => {
          settle();
          if (stale()) return;
  ...
  const ensureLevel = React.useCallback(
    (key: string, reason: CascaderLoadReason) => {
      const { getChildren: loader, store: current, base: index } = latest.current;
      if (!loader) return;
  ...
  const prefetchNode = React.useCallback(
    (node: CascaderNode<T> | null | undefined) => {
      const { prefetch: on, getChildren: loader, store: current } = latest.current;
  ...
  const runSearch = React.useCallback(
    (text: string) => {
      ...
      Promise.resolve()
        .then(() => searcher(text, { signal: controller.signal, path: currentPath }))
        .then((result) => {
          settle();
          if (stale()) return;
          const items = Array.isArray(result) ? result : result.items;
  ...
  const runResolve = React.useCallback(
    (value: string) => {
      ...
      Promise.resolve()
        .then(() => resolver(value, { signal: controller.signal, reason: 'resolve' }))
        .then((chain) => {
          settle();
...
export function useCascaderLoadState(parent?: string | null): CascaderLoadState | null {
