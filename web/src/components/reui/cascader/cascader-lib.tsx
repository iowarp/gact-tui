 ...
 */
export const CASCADER_LIST_PAD_CLASS = '[--cascader-list-pad:4px]';
...
function getCascaderTabStops(panel: HTMLElement): HTMLElement[] {
  const stops = Array.from(panel.querySelectorAll<HTMLElement>(CASCADER_TAB_STOP_SELECTOR));
  return stops.filter(
    (stop) =>
...
export function buildCascaderIndex<T = unknown>(
  ...
  if (getParent) {
    ...
    for (const node of all) {
      ...
      const parent = rawParent != null && byValue.has(rawParent) ? rawParent : null;
...
export function mergeCascaderIndex<T = unknown>(
  ...
  const insert = (parentKey: string, nodes: readonly CascaderNode<T>[]) => {
    for (const node of nodes) {
      if (byValue.has(node.value)) continue;
      byValue.set(node.value, node);
      parentOf.set(node.value, parentKey === CASCADER_ROOT_KEY ? null : parentKey);
...
export function isCascaderBranch<T>(index: CascaderIndex<T>, node: CascaderNode<T>): boolean {
...
export function getCascaderCount<T>(index: CascaderIndex<T>, node: CascaderNode<T>): number {
...
export function matchesCascaderQuery<T>(node: CascaderNode<T>, normalized: string): boolean {
...
export function filterCascaderLevel<T>(
  ...
  query: string,
  matches: (node: CascaderNode<T>, normalized: string) => boolean = matchesCascaderQuery,
): CascaderNode<T>[] {
...
export function flattenCascaderTree<T>(
  ...
  const walk = (nodes: readonly CascaderNode<T>[], parentKey: string, depth: number) => {
