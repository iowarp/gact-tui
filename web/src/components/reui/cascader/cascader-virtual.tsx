 ...
 */
function useCascaderVirtualGutter(contentElement: HTMLElement | null): CascaderVirtualGutter {
  ...
  React.useLayoutEffect(() => {
    ...
    const inline = Number.parseFloat(styles.paddingInlineStart || styles.paddingLeft) || 0;
    setGutter((previous) =>
      previous.block === block && previous.inline === inline ? previous : { block, inline },
    );
  }, [contentElement]);
 ...
 */
function cascaderScrollElement(content: HTMLElement | null): HTMLElement | null {
  if (!content) return null;
  return content.closest<HTMLElement>('[data-slot="scroll-area-viewport"]') ?? content;
}
...
function CascaderVirtualItems(props: CascaderVirtualItemsProps) {
  ...
  React.useLayoutEffect(() => registerVirtualRenderer(), [registerVirtualRenderer]);
...
function CascaderVirtualRows({ estimateSize, overscan }: CascaderVirtualItemsProps) {
  ...
  const { renderedItems, treeRows, deepResults, loadStates } = useCascaderState();
  ...
  const getItemKey = React.useCallback(
    (index: number) => (tree ? treeRows[index]?.node.value : renderedItems[index]?.value) ?? index,
...
function CascaderVirtualColumnRows({
  ...
  const getItemKey = React.useCallback((index: number) => items[index]?.value ?? index, [items]);
