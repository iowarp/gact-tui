      ...
      {actions.map((action, i) =>
        action.items?.length ? (
          <CascaderSubmenu key={actionKey(action, i)}>
            <CascaderSubmenuTrigger icon={action.icon} disabled={action.disabled}>
              {action.label}
    ...
    <>
      {runs.map((run, runIndex) =>
        run.group ? (
          ...
          <React.Fragment key={`run-${runIndex}`}>{run.items.map(renderAction)}</React.Fragment>
        ),
      )}
    </>
  );
...
function menuItems(popup: HTMLElement | null): HTMLElement[] {
  if (!popup) return [];
  return Array.from(popup.querySelectorAll<HTMLElement>('[role="menuitem"]:not([disabled])'));
}

const CascaderSubmenuContext = React.createContext<CascaderSubmenuContextValue | undefined>(
  undefined,
);
...
function CascaderSubmenuTrigger({
  ...
  const handleKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>) => {
      ...
      const openKey = isCascaderRtl(event.currentTarget, direction) ? 'ArrowLeft' : 'ArrowRight';
...
function CascaderSubmenuContent({
  ...
  const handleKeyDown = React.useCallback(
    (event: CascaderSubmenuKeyEvent) => {
      ...
      const closeKey = isCascaderRtl(event.currentTarget, direction) ? 'ArrowRight' : 'ArrowLeft';
    ...
    <PopoverPrimitive.Portal>
      ...
      >
        <PopoverPrimitive.Popup
          ...
          {...props}
        >
          <CascaderMenuContext.Provider value={true}>{children}</CascaderMenuContext.Provider>
        </PopoverPrimitive.Popup>
      </PopoverPrimitive.Positioner>
    </PopoverPrimitive.Portal>
  );
