...
interface GanttViewProps extends HTMLAttributes<HTMLDivElement> {
            ...
            return [
              {
                ...
                baseline,
                startMin: Math.max(
                  (baseline.start.getTime() - rangeStartMs) / 60000,
                  0
                ),
                endMin: Math.min(
                  (baseline.end.getTime() - rangeStartMs) / 60000,
                  totalMin
                ),
                lane,
  ...
  const timelineContent = (
    <div
      className={cn(
        ...
        // `w-max` lets axis label text override the documented zoom width,
        // which prevents compact execution ranges from fitting the viewport.
        // The header and rows already own `trackWidth`; keep this wrapper
        // viewport-sized and let those explicit widths drive horizontal scroll.
        "flex min-h-full w-full min-w-full grow flex-col",
        customScrollbars && "pb-2.5"
      )}
    >
      ...
      <div ref={bodyRef} className="relative flex min-h-0 flex-1">
        ...
        >
          ...
          {customScrollbars ? (
            ...
            <ScrollArea
              key={scale}
              className="h-full [&>[data-orientation=vertical]]:top-[65px]! [&>[data-orientation=vertical]]:bottom-4! [&>[data-orientation=vertical]]:h-auto! [&>[data-slot=scroll-area-viewport]>div]:flex! [&>[data-slot=scroll-area-viewport]>div]:min-h-full [&>[data-slot=scroll-area-viewport]>div]:flex-col"
            >
              {timelineContent}
              {horizontalScrollbar}
            </ScrollArea>
...
const GanttDependencyLayer = memo(function GanttDependencyLayer({
  ...
  const liveX = (ms: number) =>
    (Math.min(Math.max((ms - rangeStartMs) / 60000, 0), totalMin) / totalMin) *
    trackRemWidth
  return (
    <svg
      aria-hidden
      ...
      className={cn(
        "text-muted-foreground/70 pointer-events-none absolute start-0 top-0 w-full overflow-visible rtl:-scale-x-100",
    ...
    >
      {edges.map((edge) => {
        ...
        const dragged =
          dragLink !== null &&
          (edge.fromId === dragLink.eventId || edge.toId === dragLink.eventId)
        const fromX =
          dragged && edge.fromId === dragLink.eventId
            ? liveX(dragLink.endMs)
            : edge.fromX
        const toX =
          dragged && edge.toId === dragLink.eventId
            ? liveX(dragLink.startMs)
            : edge.toX
...
const GanttTimelineRow = memo(function GanttTimelineRow({
  ...
  const hintTopRem = Math.min(
    laneTopRem(laneOffsetRem, hintLane, laneHeightRem, laneGapRem) +
      laneHeightRem / 2,
      ...
      >
        ...
        {bars?.rowBaseline &&
          (() => {
            ...
            const bandRemHeight = laneBlockRem(
              laneCount,
              laneHeightRem,
              laneGapRem
            )
            ...
            return (
              <div
                ...
                style={
                  {
                    "--gantt-event-color":
                      row.resource.color ?? "var(--color-primary)",
          ...
          return (
            <div
              ...
              style={
                {
                  "--gantt-event-color":
                    plan.event.color ?? "var(--color-primary)",
        ...
        {segments.map((segment, segmentIndex) => {
          ...
          const milestone =
            segment.occurrence.end.getTime() ===
            segment.occurrence.start.getTime()
          ...
          const barRemWidth = milestone
            ? laneHeightRem
            : (to - from) * trackRemWidth
          const wantsOutside =
            milestone ||
