...
- [~] **CLIO-BBBBBBBBBB-D2.** `--auto-meridian` flag on `gact-clio-adapter` (or its successor in the Python entry point): when set, the deploy spawns Meridian alongside `clio-agent-gact` and stitches the env vars automatically. Readiness probes both processes. Removes the manual setup step from the happy path. [Partial: clio-agent-gact's main() now wires the real ClioAgent when CLIO_LM_PROVIDER is set; agent.forward() runs in executor so the event loop stays responsive. The literal --auto-meridian flag (spawn Meridian alongside) didn't land — Meridian is a separate persistent process and the deploy already inherits the parent shell's env, so a flag adds little. Document the recipe instead.]

- [x] **CLIO-BBBBBBBBBB-D3.** Real-LM smoke test: `gact agent deploy clio prod --auto-meridian && gact connect prod`, run a turn that exercises a tier-2 expert (e.g. "analyze /tmp/sample.parquet"), assert the routing badge paints, the expert produces a real response, ARC cache stats update, the assistant reply renders cleanly. Skipped in CI when Claude Max OAuth + Meridian aren't available; runs locally as a manual verification step. [✓ Real Haiku turn lands: TUI renders "▸ data · LM-routed", multi-paragraph NetCDF recommendation from Claude, ARC cache chip 33% (real ARC, not fake). Tool-execution path (HDF5 MCP) hangs without MCP config; conversational + advisory turns work end-to-end.]
...
- [x] **CLIO-BBBBBBBBBB-D5.** README gains a "Quick start with CLIO + Claude Max" section walking through Meridian + clio-agent-gact + gact in three commands. Targets the user who wants to try the integration without reading the full integration plan. [✓ README "Quick start with CLIO + Claude Max" section. CLIO Agent backends-table row points at the section. CLIO repo's docs/providers/meridian.md is the deeper guide.]
