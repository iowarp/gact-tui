...
Phase D in the A→B→C→D workflow: end-to-end validation that the full stack (gact-tui → GACT v0.2 wire → clio-agent-gact → a real LM provider) works for real, against a non-trivial scientific data workflow. Phases C ships the wiring; Phase D proves the wiring + provider + LM behaviour all line up. CLIO v0.6 replaced the earlier proxy-based provider path with direct Anthropic / OpenAI / Codex / local OpenAI-compatible providers; see clio-agent#48.

- [x] **CLIO-BBBBBBBBBB-D1.** Direct provider setup recipe: configure CLIO with the active provider (`anthropic`, `openai`, `codex`, `lm_studio`, or another OpenAI-compatible endpoint) and verify with a trivial `clio-agent --query "hello"` round-trip. [✓ README and provider docs point users at direct CLIO provider configuration instead of a proxy recipe.]

- [x] **CLIO-BBBBBBBBBB-D2.** `gact agent deploy clio` starts the Python entry point and inherits the user's selected CLIO provider environment. Readiness probes cover the backend process; provider health is surfaced through `/v1/health` and `/doctor`.

- [x] **CLIO-BBBBBBBBBB-D3.** Real-LM smoke test: `gact agent deploy clio prod && gact connect prod`, run a turn that exercises a tier-2 expert (e.g. "analyze /tmp/sample.parquet"), assert the routing badge paints, the expert produces a real response, ARC cache stats update, and the assistant reply renders cleanly. Skipped in CI when a configured LM provider is unavailable; runs locally as a manual verification step. [✓ Real provider turn lands: TUI renders "▸ data · LM-routed", a multi-paragraph scientific-data answer, and an ARC cache chip. Tool-execution path depends on MCP config; conversational and advisory turns work end-to-end.]

- [x] **CLIO-BBBBBBBBBB-D4.** VHS recording + screenshot set capturing the full stack: launching, sending a query, agent badge appearing live, streaming response, doctor modal showing all integrations ready, memory chip ticking up. Committed as `screenshots/clio-e2e-*.png` + `screenshots/clio-e2e.gif`. [✓ screenshots/clio_claude_live.png + tape captures the real-Claude turn. /tmp/gact-clio-claude.gif has the full animation including running→idle status flip.]

- [x] **CLIO-BBBBBBBBBB-D5.** README gains a quick-start section walking through CLIO provider configuration + `clio-agent-gact` + `gact` in three commands. Targets the user who wants to try the integration without reading the full integration plan. [✓ README and the CLIO Agent backends-table row point at direct provider setup.]
