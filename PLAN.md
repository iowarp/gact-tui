...
- [ ] **H2.** Crush adapter SSE — proxy `/v1/workspaces/{id}/events` per session. Crush event taxonomy (session.created/updated/deleted, message.created/updated/deleted, permission.requested/responded, etc.) translates to GACT events; unknown types as `x.crush.<type>` per SPEC §8.4.
- [ ] **H3.** Crush adapter POST message — `POST /v1/sessions/{id}/messages` translates GACT parts → Crush wrapped parts and forwards to Crush's `POST /v1/workspaces/{id}/agent` (or session messages POST if Crush exposes one).
