 ...
 * SolidJS inserts a given node reference into exactly one place — so when the
 * same icon (e.g. `palette`, `sessions`) was rendered in two spots (topbar AND
 * the rail) the shared singleton was "stolen" by whichever mounted last,
 * leaving the other `<svg>` empty (the blank-box bug). Returning a fresh tree
 * per call gives every render site its own nodes.
 */
const PATHS: Record<IconName, () => JSX.Element> = {
  ...
  /* Unified catalog / browse-everything: a grid of cards. */
  catalog: () => (
    <>
      <rect x="3" y="3" width="7" height="7" rx="1.5" />
      <rect x="14" y="3" width="7" height="7" rx="1.5" />
...
export function Icon(props: IconProps) {
  ...
  // Resolve the glyph FACTORY for this name. Degrade gracefully: an unknown
  // name renders nothing rather than an empty bordered/blank svg box.
  // (TypeScript already constrains `name` to IconName, but a stale/dynamic
  // string could slip through at runtime.)
  const factory = () => PATHS[props.name] as (() => JSX.Element) | undefined;
  return (
    <Show when={factory()}>
      {(make) => (
        <svg
