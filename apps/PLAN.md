...
window menus, code-block gutter, hljs lazy-load, MCP Reconnect restore.

Remaining follow-ups discovered during that run (not 1.0 blockers):

- **@-picker hover previews** — needs a clio endpoint that serves arbitrary
  WORKSPACE file bytes (PR #533 only serves registered context files).
  Candidate next clio PR.
- **Composer image-attachment thumbnails** — client-side object-URL preview
  on upload chips; small polish.
- **`pnpm fetch-sidecar` broken on Windows/git-bash** (Go tries to fetch a
