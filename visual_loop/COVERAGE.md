...
| Workflow/agent sidebar | `semantic_agent_workflow_sidebar.tape`, `agent_runtime_sidebar.tape`, `agents_files_sidebar.tape` | `semantic_agent_workflow_sidebar.png`, `semantic_agent_workflow_sidebar.gif`, `agent_runtime_sidebar.png` | Covered |
...
| File viewer/module | `semantic_file_viewer_module.tape`, `semantic_workspace_files_live_refresh.tape` | `semantic_file_viewer_module_initial.png`, `semantic_file_viewer_module_detail.png`, `semantic_file_viewer_module_raw.png`, `semantic_file_viewer_module_upload.png`, `semantic_workspace_files_live_refresh_before.png`, `semantic_workspace_files_live_refresh_after.png`, `file_detail_renderer_performance.report.md` | Covered |
...
| Live TUI latency partial proof | `capture_live_tui_latency.sh`, `capture_tui_mouse_latency_pty.py` | `live_clio_tui_latency_metrics.png`, `live_clio_tui_latency_capture.gif`, `live_clio_tui_latency_manifest.json`, `live_clio_tui_latency_report.json`, `live_clio_tui_mouse_latency_manifest.json`, `live_clio_tui_mouse_latency_report.json`, `tui_click_latency_target_semantics.report.md`, `tui_mouse_latency_pty_manifest.json`, `tui_mouse_latency_pty_report.json`, `copy_latency_telemetry.report.md`, `tui_latency_readiness.report.md` | Preserved owned-backend capture proving `/metrics` renders TUI interaction latency by surface on real CLIO; deterministic report/test evidence proves click rows preserve surface/input kind/target label/last hit target; PTY proof sends real terminal SGR mouse sequences and now requires section-level click latency for header, left sidebar, conversation, and input plus conversation wheel latency. The maintained readiness check gates those primary PTY sections at 125% of the checked-in p95 baseline so rendering or input regressions fail before release. Copy operations are also reported as first-class `copy` latency rows with section `copy_count`, so full-conversation and selected-block copy no longer disappear into generic key latency. The owned live mouse capture proves the TUI side works against real CLIO but records CLIO-side blockers: `session_status_idle`, `backend_metrics_sample_count_zero`, `provider_streaming_limitation`, and `live_streaming_false`. #160 remains open for active live-stream proof and long-session profiling, not for mouse-event feasibility |
| 0.8.3 release readiness | `check_release_0_8_3_readiness.py` | `release_0_8_3_readiness.report.md` | Focused gate for #150, #154, #143, and #152. Deterministic proof must stay green; `--strict-live` is reserved for the real terminal and owned-backend captures needed before tagging 0.8.3 |
...
| Live executor streaming recheck | owned CLIO backend on fixed `clio-agent#692` branch | `tui-live-streaming-fixed-midturn.png`, `tui-live-streaming-fixed-final.png`, `tui-live-streaming-fixed.gif` | Preserved 2026-06-18 ALCF/Gemma proof after CLIO `bea9cd8`: the terminal TUI receives dynamic-agent live deltas, shows an active mid-turn transcript, and reconciles to idle with final assistant metadata `stream_source: live` |
...
| Diagnostics | Real CLIO doctor output with partial capability gaps and long-running benchmark metrics during active stream (#151/#160) | Deterministic fixtures, maintained `gact diag` clipboard/terminal report, preserved live memory-pressure evidence, and PTY mouse-latency proof are covered; operators still need real CLIO doctor partial-gap and active-stream metrics captures during a long owned CLIO stream | High |
