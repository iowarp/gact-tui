...
| Live TUI latency partial proof | `capture_live_tui_latency.sh` | `live_clio_tui_latency_metrics.png`, `live_clio_tui_latency_capture.gif`, `live_clio_tui_latency_manifest.json` | Preserved owned-backend capture proving `/metrics` renders TUI interaction latency by surface on real CLIO; manifest records provider streaming limitation, so #160 remains open for active live-stream proof |
