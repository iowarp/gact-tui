...
| Live TUI latency partial proof | `capture_live_tui_latency.sh` | `live_clio_tui_latency_metrics.png`, `live_clio_tui_latency_capture.gif`, `live_clio_tui_latency_manifest.json`, `live_clio_tui_latency_report.json` | Preserved owned-backend capture proving `/metrics` renders TUI interaction latency by surface on real CLIO; the report preserves exact TUI-local samples by surface/input kind/last hit target. Manifest records provider streaming limitation, so #160 remains open for active live-stream proof |
