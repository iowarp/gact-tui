...
port="${GACT_CANCEL_FAILURE_PORT:-41950}"
backend="http://127.0.0.1:${port}"
log="${TMPDIR:-/tmp}/gact-cancel-failure.log"
config_dir="$(mktemp -d)"

.tools/emulator-server \
  -port "$port" \
  -timing fast \
  -cancel-failures >"$log" 2>&1 &
srv=$!
tui_pid=""
cleanup() {
  if [ -n "${tui_pid:-}" ]; then
...
session_id="$(curl -fsS -X POST "${backend}/v1/sessions" \
  ...
  -d '{"workspace_id":"ws_default","title":"cancel failure demo","agent":{"id":"seismic_waveform"}}' |
