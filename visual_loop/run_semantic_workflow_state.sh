...
port="${GACT_SEMANTIC_WORKFLOW_STATE_PORT:-41917}"
backend="http://127.0.0.1:${port}"
session_id="ses_seed_ws_default_1"
log="${TMPDIR:-/tmp}/gact-semantic-workflow-state.log"
config_dir="$(mktemp -d)"
...
(
  ...
  curl -fsS -X POST "${backend}/v1/sessions/${session_id}/messages" \
    ...
    -d '{"parts":[{"type":"text","text":"workflow state semantic demo"}]}' >/dev/null
) &
