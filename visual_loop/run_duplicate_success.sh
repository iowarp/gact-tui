...
port="${GACT_DUPLICATE_SUCCESS_PORT:-41948}"
backend="http://127.0.0.1:${port}"
log="${TMPDIR:-/tmp}/gact-duplicate-success.log"
config_dir="$(mktemp -d)"
...
session_id="$(curl -fsS -X POST "${backend}/v1/sessions" \
  ...
  -d '{"workspace_id":"ws_default","title":"seismic review","agent":{"id":"seismic_waveform"}}' |
