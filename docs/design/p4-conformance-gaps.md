...
| B6 | [clio-agent#1175](https://github.com/iowarp/clio-agent/issues/1175) — no **update-check endpoint**, so the version stamp shows the version but cannot claim "update available" as the prototype does. Asserting it would be inventing a fact. | open |
