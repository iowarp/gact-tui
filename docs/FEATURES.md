# GACT — full feature reference

This is the long-form companion to the [root README](../README.md). It covers
every CLI subcommand, every keybinding, the full SPEC capability matrix, and
the project layout. The root README is intentionally short so the front door
loads fast; everything that doesn't fit in a 2-minute scan lives here.
...
│   └── conformance/     # runnable conformance suite (any backend)
├── emulator/
